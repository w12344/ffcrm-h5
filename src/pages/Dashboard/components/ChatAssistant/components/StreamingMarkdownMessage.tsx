...
const StreamingMarkdownMessage: React.FC<StreamingMarkdownMessageProps> = ({ message }) => {
    ...
    const animate = () => {
      ...
      if (timeSinceLastUpdate >= updateInterval) {
        ...
        if (currentContentLength < targetContentLength) {
          ...
          // 检查剩余内容是否包含markdown格式
          const remainingContent = targetContentRef.current.substring(currentContentLength);
          const hasMarkdownFormat = /^[#*`_\[\]()\s\n]/.test(remainingContent);
          
          // 检查是否正在处理标题行
          const isProcessingHeading = /^#{1,6}\s+/.test(remainingContent);
          
          // 对于markdown格式，使用更保守的批量处理
          const charsToAdd = hasMarkdownFormat 
            ? Math.min(remainingChars > 50 ? 3 : 1, remainingChars) // 对markdown格式更保守
            : Math.min(remainingChars > 200 ? 12 : remainingChars > 50 ? 6 : 3, remainingChars);
          
          // 如果正在处理标题，确保一次性显示完整的标题行
          let newIndex = currentContentLength + charsToAdd;
          if (isProcessingHeading) {
            // 找到标题行的结束位置
            const headingEndIndex = targetContentRef.current.indexOf('\n', currentContentLength);
            if (headingEndIndex !== -1) {
              newIndex = Math.min(newIndex, headingEndIndex + 1);
            }
          }
